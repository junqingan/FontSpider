...
gulp.task('html', function() {
	gulp.src(['./static/src/html/_index.html'])
		.pipe(fileinclude())
...
gulp.task('web', function() {
  gulp.src('')
    .pipe(webserver({
      ...
      open: 'static/dist/_index.html',
    }));
});

gulp.task('default', ['css', 'js','images', 'html', 'font']);
gulp.task('watch', function () {
    gulp.watch('static/**/*.css', ['css','html']); //当所有css文件发生改变时，调用css任务
});